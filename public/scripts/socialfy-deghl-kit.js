...
(function socialfyDeGhlKit() {
  ...
  function markSubnavContrast() {
    ...
    for (var i = 0; i < candidates.length; i++) {
      ...
      var styles = window.getComputedStyle ? window.getComputedStyle(item) : null;
      if (!selected && styles) {
        var bg = styles.backgroundColor || '';
        selected = /rgb\(7, 17, 38\)|rgb\(2, 6, 23\)|rgba\(2, 6, 23|rgb\(15, 23, 42\)/.test(bg);
      }
      if (!selected && rect.width > 40 && rect.height > 24 && text && (text === 'Conversas' || text.indexOf('Visualizar') === 0 || text === 'Funis')) selected = true;
