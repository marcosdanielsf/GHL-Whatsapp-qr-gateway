...
export function InstanceList({
  ...
  const filteredInstances = useMemo(() => {
    ...
    return instances.filter((instance) => {
      const matchesId =
        !term || instance.instanceId.toLowerCase().includes(term);
  ...
  const handleReconnect = async (instanceId: string) => {
    try {
      ...
      toast.error(`${t("reconnectError")} ${instanceId}`);
    } finally {
  ...
  const STATUS_FILTERS: Array<{
    value: "" | "ONLINE" | "RECONNECTING" | "OFFLINE";
    label: string;
  }> = [
    { value: "", label: t("all") },
            ...
            <div className={`select-status ${isMenuOpen ? "open" : ""}`}>
              ...
              >
                ...
                <span className="label">
                  {STATUS_FILTERS.find((o) => o.value === statusFilter)
                    ?.label || t("all")}
                </span>
                <span className={`status-glyph ${statusFilter || "all"}`}>
                  {getStatusGlyphIcon(statusFilter)}
                </span>
          ...
          <div className="queue-meter-row">
            ...
            {queueStatsUpdatedAt && (
              <span className="queue-updated">
                {t("lastUpdate")}:{" "}
                {new Date(queueStatsUpdatedAt).toLocaleTimeString(
                  language === "es"
                    ? "es-PE"
                    : language === "pt"
                      ? "pt-BR"
